        ...
        private sealed class QuerySuggest : Query
        {
            ...
            public List<string> Suggest(ReadOnlySpan<char> word)
            {
                ...
                else if (capType == CapitalizationType.Init)
                {
                    ...
                    good |= Suggest(slst, HunspellTextFunctions.MakeAllSmall(scw, Affix.Culture), ref onlyCompoundSuggest);
                ...
                if (!good && Affix.MaxNgramSuggestions != 0 && (slst.Count == 0 || onlyCompoundSuggest))
                {
                    ...
                    else if (capType == CapitalizationType.Huh || capType == CapitalizationType.HuhInit)
                    {
                        ...
                        NGramSuggest(slst, HunspellTextFunctions.MakeAllSmall(scw, Affix.Culture), CapitalizationType.Huh);
                    }
                    else if (capType == CapitalizationType.Init)
                    {
                        capWords = true;
                        NGramSuggest(slst, HunspellTextFunctions.MakeAllSmall(scw, Affix.Culture), capType);
                    }
                    else if (capType == CapitalizationType.All)
                    {
                ...
                if (dashPos >= 0)
                {
                    ...
                    while (!good && noDashSug && !last)
                    {
                        ...
                        var chunk = scw.Slice(prevPos, dashPos - prevPos);
                        if (!Check(chunk))
                        {
                ...
                if (Affix.OutputConversions.HasReplacements)
                {
                    for (var j = 0; j < slst.Count; j++)
                    {
                        if (Affix.OutputConversions.TryConvert(slst[j].AsSpan(), out string wspace))
                        {
                            slst[j] = wspace;
                        }
                    }
            ...
            private int DoubleTwoChars(List<string> wlst, ReadOnlySpan<char> word, bool cpdSuggest)
            {
                ...
                for (var i = 2; i < word.Length; i++)
                {
                    if (word[i] == word[i - 2])
                    {
                        ...
                        {
                            TestSug(wlst, word.Slice(0, i - 1).ConcatString(word.Slice(i + 1)), cpdSuggest);
                            state = 0;
                        }
                    }
                    else
            ...
            private int ExtraChar(List<string> wlst, ReadOnlySpan<char> word, bool cpdSuggest)
            {
                ...
                var builder = word.ToArray().AsSpan();

                for (var index = word.Length - 1; index >= 0; index--)
                {
                    word.Slice(index + 1).CopyTo(builder.Slice(index));

                    TestSug(wlst, builder.Slice(0, builder.Length - 1), cpdSuggest);

                    word.Slice(index).CopyTo(builder.Slice(index));
                }
            ...
            private void CapChars(List<string> wlst, ReadOnlySpan<char> word, bool cpdSuggest) =>
                TestSug(wlst, HunspellTextFunctions.MakeAllCap(word, Affix.Culture), cpdSuggest);

            private int MapChars(List<string> wlst, ReadOnlySpan<char> word, bool cpdSuggest)
            {
            ...
            private int MapRelated(ReadOnlySpan<char> word, ref string candidate, int wn, List<string> wlst, bool cpdSuggest, OperationTimeLimiter timer)
            {
                if (wn >= word.Length)
                {
                    if (
                ...
                foreach (var mapEntry in Affix.RelatedCharacterMap)
                {
                    ...
                    {
                        if (word.Slice(wn).StartsWith(mapEntryValue.AsSpan()))
                        {
                            inMap = true;
                ...
                if (!inMap)
                {
                    candidate = candidate.ConcatString(word[wn]);
            ...
            private int CheckWord(ReadOnlySpan<char> word, bool cpdSuggest, OperationTimeLimiter timer = null)
            {
                ...
                if (rvDetails.Length != 0)
                {
                    ...
                    rv = rvDetail?.ToEntry(wordString);
                }
                else
                {
                    rv = PrefixCheck(wordString, CompoundOptions.Not, default); // only prefix, and prefix + suffix XXX
                }

                var noSuffix = rv != null;
                if (!noSuffix)
                {
                    rv = SuffixCheck(wordString, AffixEntryOptions.None, default, default, default, CompoundOptions.Not); // only suffix
                }

                if (Affix.ContClasses.HasItems && rv == null)
                {
                    rv = SuffixCheckTwoSfx(wordString, AffixEntryOptions.None, default, default)
                        ?? PrefixCheckTwoSfx(wordString, CompoundOptions.Not, default);
                }
            ...
            /// </summary>
            private int ReplChars(List<string> wlst, ReadOnlySpan<char> word, bool cpdSuggest)
            {
                ...
                foreach (var replacement in WordList.AllReplacements)
                {
                    ...
                    var patternSpan = replacement.Pattern.AsSpan();

                    // search every occurence of the pattern in the word
                    for (
                        var r = word.IndexOf(patternSpan)
                        ;
                        r >= 0
                        ; 
                        r = word.IndexOf(patternSpan, r + 1) // search for the next letter
                    )
                    {
                        var type = (r == 0) ? ReplacementValueType.Ini : ReplacementValueType.Med;
                        if (r + patternSpan.Length == word.Length)
                        {
                        ...
                        {
                            var candidate = word.Slice(0, r).ConcatString(replacement[type], word.Slice(r + patternSpan.Length));
            ...
            /// </summary>
            private void NGramSuggest(List<string> wlst, ReadOnlySpan<char> word, CapitalizationType capType)
            {
                int lval;
                int sc;
                ...
                var target = hasPhoneEntries
                    ? Phonet(HunspellTextFunctions.MakeAllCap(word, Affix.Culture))
                    : string.Empty;
                ...
                }, maxAllowedLength))
                {
                    ...
                    foreach (var hpDetail in hpSet.Value)
                    {
                        ...
                        sc = NGram(3, word, hpSet.Key.Span, NGramOptions.LongerWorse | NGramOptions.Lowering)
                            + LeftCommonSubstring(word, hpSet.Key.Span);

                        // check special pronunciation
                        if (EnumEx.HasFlag(hpDetail.Options, WordEntryOptions.Phon) && CopyField(out var f, hpDetail.Morphs, MorphologicalTags.Phon))
                        {
                            var sc2 = NGram(3, word, f.Span, NGramOptions.LongerWorse | NGramOptions.Lowering)
                                + LeftCommonSubstring(word, f.Span);
                        ...
                        {
                            scphon = NGram(3, target.AsSpan(), Phonet(HunspellTextFunctions.MakeAllCap(hpSet.Key.Span, Affix.Culture)).AsSpan(), NGramOptions.LongerWorse) * 2;
                        }

                        if (sc > roots[lp].Score)
                        {
                            roots[lp].Score = sc;
                            roots[lp].Root = new WordEntry(hpSet.Key.ToString(), hpDetail);
                            lval = sc;
                ...
                var thresh = 0;
                var mw = new char[word.Length].AsSpan();
                for (var sp = 1; sp < 4; sp++)
                {
                    ...
                    thresh += NGram(word.Length, word, mw, NGramOptions.AnyMismatch | NGramOptions.Lowering);
                }
                ...
                for (var i = 0; i < roots.Length; i++)
                {
                    ...
                    if (rp != null)
                    {
                        if (!EnumEx.HasFlag(rp.Detail.Options, WordEntryOptions.Phon) || !CopyField(out var field, rp.Detail.Morphs, MorphologicalTags.Phon))
                        {
                            field = null;
                        }
                        ...
                        for (var k = 0; k < nw; k++)
                        {
                            ...
                            sc = NGram(word.Length, word, guessWordK.Word.Span, NGramOptions.AnyMismatch | NGramOptions.Lowering)
                                + LeftCommonSubstring(word, guessWordK.Word.Span);
                ...
                {
                    var guess = guesses[i];
                    if (!guess.Guess.IsEmpty)
                    {
                        // lowering guess[i]
                        var gl = HunspellTextFunctions.MakeAllSmall(guess.Guess.Span, Affix.Culture);
                ...
                if (hasPhoneEntries)
                {
                    ...
                    {
                        var root = roots[i];
                        if (!root.RootPhon.IsEmpty)
                        {
                            // lowering rootphon[i]
                            var gl = HunspellTextFunctions.MakeAllSmall(root.RootPhon.Span, Affix.Culture);
                    ...
                    if (!guess.Guess.IsEmpty)
                    {
                        ...
                        )
                        {
                            ...
                            for (var j = 0; j < wlst.Count; j++)
                            {
                                ...
                                // prefixes or affixes
                                if (
                                    (guess.GuessOrig == null && guess.Guess.Span.Contains(wlst[j].AsSpan()))
                                    ||
                            ...
                            if (unique)
                            {
                                wlst.Add(guess.GuessOrig ?? guess.Guess.ToString());
                            }
                        }
                ...
                if (hasPhoneEntries)
                {
                    for (var i = 0; i < roots.Length; i++)
                    {
                        ...
                        if (!root.RootPhon.IsEmpty && wlst.Count < wlstLimit)
                        {
                            var unique = true;
                            for (var j = 0; j < wlst.Count; j++)
                            {
                            ...
                            if (unique)
                            {
                                wlst.Add(root.RootPhon.ToString());
                            }
                        }
                    }
                }
            ...
            private int CommonCharacterPositions(ReadOnlySpan<char> s1, ReadOnlySpan<char> s2, ref bool isSwap)
            {
                // decapitalize dictionary word
                var t = Affix.ComplexPrefixes
                    ? s2.Slice(0, s2.Length - 1).ConcatString(TextInfo.ToLower(s2[s2.Length - 1])).AsSpan()
                    : HunspellTextFunctions.MakeAllSmall(s2, Affix.Culture);
            ...
            /// </summary>
            private static void Lcs(ReadOnlySpan<char> s, ReadOnlySpan<char> s2, out int m, out int n, out LongestCommonSubsequenceType[] b)
            {
                m = s.Length;
                n = s2.Length;
                var nNext = n + 1;
            ...
            private int ExpandRootWord(GuessWord[] wlst, WordEntry entry, ReadOnlySpan<char> bad, ReadOnlyMemory<char> phon)
            {
                var nh = 0;
                // first add root word to list
                if (nh < wlst.Length && !entry.Detail.ContainsAnyFlags(Affix.NeedAffix, Affix.OnlyInCompound))
                {
                    wlst[nh].Word = entry.Word.AsMemory();
                    if (wlst[nh].Word.IsEmpty)
                    {
                        return 0;
                    }
                    ...
                    if (!phon.IsEmpty && nh < wlst.Length)
                    {
                        wlst[nh].Word = phon;
                        if (wlst[nh].Word.IsEmpty)
                        {
                            ...
                            if (
                                (
                                    ...
                                    (
                                        ...
                                        &&
                                        bad.Slice(bad.Length - key.Length).EqualsOrdinal(sptr.Append.AsSpan())
                                    )
                                )
                            ...
                            )
                            {
                                ...
                                if (newword.Length != 0)
                                {
                                    if (nh < wlst.Length)
                                    {
                                        ...
                                        if (!phon.IsEmpty && nh < wlst.Length)
                                        {
                                            wlst[nh].Word = (phon + key.Reverse()).AsMemory();
                                            if (wlst[nh].Word.IsEmpty)
                                            {
                                                return nh - 1;
                                            }
                                ...
                                if (
                                    ...
                                    (
                                        ...
                                        (
                                            ...
                                            &&
                                            bad.StartsWith(cptr.Key.AsSpan())
                                        )
                                    )
                                )
                                {
                                    var newword = Add(cptr, wlst[j].Word.ToString());
                                    if (newword.Length != 0)
                                    {
                                        if (nh < wlst.Length)
                                        {
            ...
            private bool CheckForbidden(ReadOnlySpan<char> word)
            {
                ...
                var wordString = word.ToString();
                if (PrefixCheck(wordString, CompoundOptions.Begin, default) == null)
                {
                    rv = SuffixCheck(wordString, AffixEntryOptions.None, default, default, default, CompoundOptions.Not)?.Detail; // prefix+suffix, suffix
                }
            ...
            private bool CopyField(out ReadOnlyMemory<char> dest, MorphSet morphs, string var)
            {
                if (morphs.Count != 0)
                {
                    ...
                    if (morph.Length != 0)
                    {
                        ...
                        if (pos >= 0)
                        {
                            ...
                            var readBuffer = morph.AsSpan(begOffset);
                            var writeBuffer = new char[morph.Length - begOffset];

                            for (var i = 0; i < readBuffer.Length; i++)
                            {
                                ref readonly var c = ref readBuffer[i];
                                if (c == ' ' || c == '\t' || c == '\n')
                                {
                                    break;
                                }
                            ...
                            dest = writeBuffer.AsMemory();
                            return true;
                        }
                    }
                }

                dest = ReadOnlyMemory<char>.Empty;
                return false;
            }
            ...
            /// </summary>
            private int LeftCommonSubstring(ReadOnlySpan<char> s1, ReadOnlySpan<char> s2)
            {
                if (Affix.ComplexPrefixes)
                {
...
#endif
            private static int LeftCommonSubstringComplex(ReadOnlySpan<char> s1, ReadOnlySpan<char> s2) =>
                (
                    s1.Length != 0
                    &&
                    s2.Length != 0
            ...
            /// </remarks>
            private string Phonet(ReadOnlySpan<char> inword)
            {
                ...
                var word = StringBuilderPool.Get(inword.Limit(MaxPhoneTUtf8Len));
            ...
            private struct NGramSuggestSearchRoot
            {
                ...
                public ReadOnlyMemory<char> RootPhon;
            ...
            private struct NGramGuess
            {
                ...
                public ReadOnlyMemory<char> Guess;
            ...
            {
                public ReadOnlyMemory<char> Word;
